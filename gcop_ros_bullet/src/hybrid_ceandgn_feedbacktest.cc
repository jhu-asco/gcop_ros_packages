...
#include "gcop_comm/Trajectory_req.h"//msg for publishing ctrl trajectory
...
bool feedback_optimize(gcop_comm::Trajectory_req::Request &req, gcop_comm::Trajectory_req::Response &resp)
{
  // Create a struct with car state:
  Bulletrccar::CarState state;
  geometry_msgs::Transform &basepose = req.itreq.x0.basepose;
  geometry_msgs::Twist &basetwist = req.itreq.x0.basetwist;
  state.cartransform.setOrigin(btVector3(basepose.translation.x, basepose.translation.y, basepose.translation.z + 0.3));//Add 0.3 to z dirxn to move the car above the 
  state.cartransform.setRotation(btQuaternion(basepose.rotation.x, basepose.rotation.y, basepose.rotation.z, basepose.rotation.w));
  state.carlinearvel = btVector3(basetwist.linear.x, basetwist.linear.y, basetwist.linear.z);
  state.carangularvel = btVector3(basetwist.angular.x, basetwist.angular.y, basetwist.angular.z);
  ...
  {
    int count_gn = round(req.itreq.t0/h);
    if(count_gn >= us.size())
    {
      cout<<"t0 is greater than problem tf"<<endl;
      cout<<"t0: "<<req.itreq.t0<<endl;
      cout<<"tf: "<<(ce->cost.tf)<<endl;
      return false;
    }
    //set x0 for gn based on current car state
    sys->setinitialstate(state, xs_gn[0]);
    //getchar();
    int us_index = 0;
    for(int count_us = 0; count_us < N_gn; count_us++)
    {
      us_index = (count_gn + count_us);
      if(us_index < us.size())
        us_gn[count_us] = us[us_index];
      else
        us_gn[count_us] = Vector2d::Zero();
    ...
    for(int count_xs = 0;count_xs <= N_gn; count_xs++)
    {
      int xs_index = (count_gn + count_xs);
      cout<<"xs_gn["<<count_xs<<"]: "<<xs_gn[count_xs].transpose()<<endl;
      if(xs_index < xs.size())
      {
        cout<<"xsdiff["<<xs_index<<"]: "<<(xs[xs_index].transpose() - xs_gn[count_xs].transpose())<<endl;
    ...
    //getchar();
    //iterate gn and check gn cost. 
    for(int count = 0;count < 3;count++)//Increase this if u want to do more iterations
    {
      gn->Iterate();
      cout<<"Optimal Cost: "<<(gn->J)<<endl;
    }
...
int main(int argc, char** argv)
{
  ...
  //Setup service:
  feedback_service = nh.advertiseService("optimization_request",feedback_optimize);

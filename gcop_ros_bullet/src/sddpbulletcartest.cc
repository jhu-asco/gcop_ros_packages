...
void ParamreqCallback(gcop_ros_bullet::CEInterfaceConfig &config, uint32_t level) 
{
  if(config.iterate)
  {
    ...
    //Publish the cost of the initial trajectory:
    {
      ...
      for(count_cost = 0;count_cost < N;count_cost++)
      {
        ...
        cost1 += (ddp->cost).L(ts[count_cost], xs[count_cost], us[count_cost], h, 0);
      }
  ...
  if(config.animate)
  {
    ...
    for(int count1 = 0;count1 < us.size();count1++)
    {
      ...
      //cout<<"Axle_dirxn: "<<(sys->m_vehicle->m_axle[0].y())<<"; "<<(sys->m_vehicle->m_axle[0].x())<<"; "<<(sys->m_vehicle->m_axle[0].z())<<endl;
      //cout<<"ForwardWS_dirxn: "<<(sys->m_vehicle->m_axle[0].y())<<"; "<<(sys->m_vehicle->m_axle[0].x())<<"; "<<(sys->m_vehicle->m_axle[0].z())<<endl;
      ...
      cout<<"ddp->sys.x" <<ts[count1+1]<<"\txs: "<<(sys->x).transpose()<<endl;//#DEBUG
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < N/2; ++i) {
    //us[i] = Vector2d(0.2, 0);
    //us[N/2+i] = Vector2d(-0.2, 0);    
    //us[i] = Vector2d(1.0, 0);
    //us[N/2+i] = Vector2d(1.0, 0);    
    us[i] = us_firsthalf;
  ...
  */
  ...
  ddp.reset(new RccarDdp(*sys, cost, ts, xs, us));
  ddp->external_render = &render_trajectory;

  nh.getParam("Ns", ddp->Ns);

  ddp->debug = true; 
  ddp->duscale = du_scale;//Set du_scale
  //ddp->dxscale = Vector4d(0.1, 0.1, 0.02, 0.05);
  //ddp->dxscale = Vector4d(0.02, 0.02, 0.01, 0.02);
  ddp->dxscale = dx_scale;
  //ddp->dxscale = Vector4d(0.002, 0.002, 0.001, 0.0);
  //nh.getParam("sample_dx0",(ddp->sample_dx0));

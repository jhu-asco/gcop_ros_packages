...
using namespace gcop_ros_utils;
...
void SE3::log(Vector6d &v, const Affine3d &g) const
{
  Vector3d w;
  so3.log(w, g.affine().topLeftCorner<3,3>());
  v.head<3>() = w;
  Matrix3d Di;
  so3.dexpinv(Di, w);
  v.tail<3>() = Di*g.affine().topRightCorner<3,1>();
}

...
void joint_publish(const gcop_comm::CtrlTraj::ConstPtr& trajectory)
{
	...
	goal.header.frame_id = "usv_origin";
	goal.header.stamp = ros::Time::now();
	goal.ns ="goalmarker";
	goal.id = 2;
	goal.type = visualization_msgs::Marker::ARROW;
	goal.action = visualization_msgs::Marker::ADD;
	goal.pose.position.x = trajectory->finalgoal.statevector[1];
	goal.pose.position.y = trajectory->finalgoal.statevector[2];
	goal.pose.position.z = 0.1;
	goal.pose.orientation = tf::createQuaternionMsgFromYaw(trajectory->finalgoal.statevector[0]);
	goal.scale.x = trajectory->finalgoal.statevector[4];
	goal.scale.y = trajectory->finalgoal.statevector[5];
	goal.scale.z = 0.05;
	...
	global_trans.transform.translation.x = trajectory->statemsg[0].statevector[1];
	global_trans.transform.translation.y = trajectory->statemsg[0].statevector[2];
	global_trans.transform.translation.z = .1;
	global_trans.transform.rotation = tf::createQuaternionMsgFromYaw(trajectory->statemsg[0].statevector[0]);

	broadcaster->sendTransform(global_trans);//send 0 posn

	finalgoal_trans.header.stamp = ros::Time::now();
	finalgoal_trans.transform.translation.x = trajectory->finalgoal.statevector[1];
	finalgoal_trans.transform.translation.y = trajectory->finalgoal.statevector[2];
	finalgoal_trans.transform.translation.z = .1;
	finalgoal_trans.transform.rotation = tf::createQuaternionMsgFromYaw(trajectory->finalgoal.statevector[0]);
	...
	for(int i =0;i<(trajectory->N)+1; i++)
	{
		...
		line_strip.points[i].x = trajectory->statemsg[i].statevector[1];
		line_strip.points[i].y =  trajectory->statemsg[i].statevector[2];
	...
	if(animate)
	{
		...
		for(int i = 1;i<(trajectory->N)+1; i++)
		{
			...
			// update transform
			global_trans.header.stamp = ros::Time::now();
			global_trans.transform.translation.x = trajectory->statemsg[i].statevector[1];
			global_trans.transform.translation.y = trajectory->statemsg[i].statevector[2];
			global_trans.transform.translation.z = .1;
			global_trans.transform.rotation = tf::createQuaternionMsgFromYaw(trajectory->statemsg[i].statevector[0]);
...
int main(int argc, char** argv) {
	...
	traj_sub = n.subscribe("/usvnode/ctrltraj",1, joint_publish);
	...
	global_trans.header.frame_id = "usv_origin";
	...
	finalgoal_trans.header.frame_id = "usv_origin";
	...
	line_strip.header.frame_id = "/usv_origin";
	line_strip.ns = "traj";
	...
	marker.header.frame_id = "/usv_origin";
